...
`/v1/video/status/{job_id}`는 개발/레거시 확인용입니다. 운영 상태의 source of truth는 WAS DB이며, frontend는 ai-engine 상태 API를 직접 호출하지 않습니다.
...
`GET /v1/video/status/{job_id}`는 개발/레거시 확인용이며 Swagger에서 deprecated로 표시됩니다. 운영 상태의 source of truth는 WAS입니다.
